function fiztrak(armature: Bone[], target: Vector, rotationSpeed: number | number[]) {
    ...
    for (let i = 0; i < armature.length; i++) {
        let diff = clipAngle(
            target.sub(armature[i].start).angle - armature[armature.length - 1].end.sub(armature[i].start).angle
        );
        let maxDiff: number;
